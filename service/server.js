...
var catchFirstUrl = 'http://dushu.xiaomi.com',	//入口页面
	deleteRepeat = {},	//去重哈希数组
	urlsArray = [],	//存放爬取网址
	catchDate = [],	//存放爬取数据
	pageUrls = [],	//存放收集文章页面网站
...
// }
pageUrls=['http://dushu.xiaomi.com/#page=main&tab=0','http://dushu.xiaomi.com/#page=topic&type=fiction&id=11059&_t=&&from=main']
	...
	superagent.get(url)
		.end(function(err,ares){
	      ...
	      return;
			...
			infoArray.title=$('h3.title').text();
			infoArray.cover=$('.book-cover img').attr('src');
		  	...
		  	superagent.get(url)
			      ...
			      //1、收集用户个人信息，昵称、园龄、粉丝、关注
						//var currentBlogApp = $('script').eq(1).text().split(',')[0].split('=')[1].trim().replace(/'/g,""),
							...
							var articleId=url.split('&')[1].split('=')[1];

						console.log('articleId is '+ articleId + '\n'); 

						res.write('the article title is :'+$('h3.title').text() +'<br/>');

						var flag = 	isRepeat(articleId);
						
						if(!flag){
							var appUrl = "http://dushu.xiaomi.com/#page=book&source_id="+ articleId +"&source=2";
							personInfo(appUrl);
						}
		  		});
      ...
      async.mapLimit(articleUrls, 5 ,function (url, callback) {
		...
		pageUrls.forEach(function(pageUrl){
			superagent.get(pageUrl)
		      ...
		      var curPageUrls = $('.u-book');
		      for(var i = 0 ; i < curPageUrls.length ; i++){
		      	var articleUrl = curPageUrls.eq(i).attr('data-href');
